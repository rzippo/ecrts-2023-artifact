...
public class CurveLowerPseudoInverseOverInterval
{
    ...
    {
        foreach (var testCase in CurveLowerPseudoInverse.ContinuousTestCases())
            yield return new object[] { testCase[0] };

        foreach (var testCase in CurveLowerPseudoInverse.LeftContinuousTestCases())
            yield return new object[] { testCase[0] };

        foreach (var testCase in CurveLowerPseudoInverse.RightContinuousTestCases())
            yield return new object[] { testCase[0] };

        foreach (var testCase in CurveLowerPseudoInverse.CornerTestCases())
            yield return new object[] { testCase[0] };
