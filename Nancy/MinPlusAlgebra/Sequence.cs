...
public sealed class Sequence : IEquatable<Sequence>, IToCodeString
{
    ...
    public static Sequence Convolution(
        ...
        bool skipIsomorphismCutEndCheck = false)
    {
        ...
        if (
            ...
            a.IsLeftContinuous && b.IsLeftContinuous &&
            a.IsNonDecreasing && b.IsNonDecreasing
        )
        {
            var ta_a = a.DefinedFrom;
            var ta_b = b.DefinedFrom;
            var tb_a = a.DefinedUntil;
            var tb_b = b.DefinedUntil;

            var la = tb_a - ta_a;
            var lb = tb_b - ta_b;
            var length = Rational.Min(la, lb);
            var cutStart = ta_a + ta_b;
            var equivCutEnd = ta_a + ta_b + length;

            bool cutEndCheck;
            if(skipIsomorphismCutEndCheck)
            {
                cutEndCheck = true;
            }
            else
            {
                cutEndCheck = cutEnd < equivCutEnd;
            }

            if (cutEndCheck)
            {
                var aConstants = a.Elements.Count(e => e is Segment {IsConstant: true});
                var bConstants = b.Elements.Count(e => e is Segment {IsConstant: true});
                var aJumps = a.EnumerateBreakpoints()
                    .Count(bp => bp.right != null && bp.center.Value != bp.right.RightLimitAtStartTime);
                var bJumps = b.EnumerateBreakpoints()
                    .Count(bp => bp.right != null && bp.center.Value != bp.right.RightLimitAtStartTime);

                var constants = aConstants + bConstants;
                var jumps = aJumps + bJumps;
                if (constants > jumps)
                {
                    var ta_f_prime = a.IsRightContinuousAt(ta_a) && a.GetSegmentAfter(ta_a).IsConstant
                        ? a.GetSegmentAfter(ta_a).EndTime
                        : ta_a;
                    var ta_g_prime = b.IsRightContinuousAt(ta_b) && b.GetSegmentAfter(ta_b).IsConstant
                        ? b.GetSegmentAfter(ta_b).EndTime
                        : ta_b;

                    var a_upi = a.UpperPseudoInverse(false);
                    var b_upi = b.UpperPseudoInverse(false);
                    ...
                    Sequence inversion;
                    if (ta_f_prime == ta_a && ta_g_prime == ta_b)
                    {
                        inversion = inversion_raw;
                    }
                    else
                    {
                        // todo: does not handle left-open sequences
                        var ext = Sequence.Constant(
                            a.ValueAt(ta_a) + b.ValueAt(ta_b),
                            ta_a + ta_b,
                            ta_f_prime + ta_g_prime
                        );
                        inversion = Sequence.Minimum(ext, inversion_raw, false); //.Cut(cutStart, (Rational) cutEnd);
                    }

                    if (cutCeiling == Rational.PlusInfinity)
                        return inversion;
                    else
                        return inversion.Elements
        ...
        if (a.IsPlusInfinite || b.IsPlusInfinite)
        {
        ...
        bool isCeilingIncluded = false)
    {
        ...
        if (a.IsMinusInfinite || b.IsMinusInfinite)
        {
            var start = a.DefinedFrom + b.DefinedFrom;
            var end = Rational.Min(a.DefinedUntil + b.DefinedUntil, cutEnd.Value);
